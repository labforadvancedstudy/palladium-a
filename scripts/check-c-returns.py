...
The source-side support is src/codegen/mod.rs:2167-2175 (`generate_block`) and
src/codegen/mod.rs:2217- (`generate_statement`, leaf arms ending `";\n"` at
...
def unmodelled_construct(items):
    ...
    for item in items:
        if item[0] == "stmt":
            ...
            # (src/codegen/mod.rs:2167-2175) writes an indent and delegates to
            # `generate_statement` (src/codegen/mod.rs:2217-), whose leaf arms
