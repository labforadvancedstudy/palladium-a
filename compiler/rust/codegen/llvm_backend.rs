...
use crate::ast::{Program, Item, Function, Type, Stmt, Expr};
...
impl LLVMCodeGenerator {
    ...
    fn generate_statement(&self, stmt: &Stmt, var_counter: &mut i32, label_counter: &mut i32) -> Result<String> {
        let mut ir = String::new();
            ...
            Expr::Call { func, args, .. } => {
                if let Expr::Ident(func_name) = func.as_ref() {
                    match func_name.as_str() {
                        "print" => {
                            if args.len() == 1 {
                                let (arg_ir, arg_var) = self.generate_expression(&args[0], var_counter)?;
                                ir.push_str(&arg_ir);
                        ...
                        _ => {
                            ...
                            for arg in args {
                                let (arg_ir, arg_var) = self.generate_expression(arg, var_counter)?;
                                ir.push_str(&arg_ir);
