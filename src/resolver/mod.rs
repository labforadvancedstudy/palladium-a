...
impl ModuleResolver {
    ...
    pub fn resolve_program(&mut self, program: &Program) -> Result<HashMap<String, ModuleInfo>> {
        ...
        for import in &program.imports {
            ...
            if let Some(module_info) = self.loaded_modules.get(&module_name) {
                resolved.insert(module_name, module_info.clone());
            }
        }
    ...
    fn load_module(&mut self, module_name: &str) -> Result<()> {
        ...
        let module_file = module_file.ok_or_else(|| {
            CompileError::Generic(format!("Module '{}' not found", module_name))
        })?;
        
        // Read and parse the module
        let source = fs::read_to_string(&module_file)
            .map_err(|e| CompileError::Generic(format!("Failed to read module '{}': {}", module_name, e)))?;
        ...
        for item in &ast.items {
            match item {
                ...
                crate::ast::Item::Enum(enum_def) => {
                    // TODO: Add visibility to EnumDef
                    // For now, assume all enums are public
