...
impl LspServer {
    ...
    fn read_message(&self, reader: &mut BufReader<std::io::Stdin>) -> Result<Option<String>> {
        ...
        let content_length = headers.get("Content-Length")
            ...
            .ok_or_else(|| crate::errors::CompileError::Generic("Missing Content-Length header".to_string()))?;
        
        // Read content
        let mut content = vec![0u8; content_length];
        reader.read_exact(&mut content)?;
        
        Ok(Some(String::from_utf8(content).map_err(|e| crate::errors::CompileError::Generic(e.to_string()))?))
    }
    ...
    fn process_message(&mut self, message: &str) -> Result<Option<String>> {
        ...
        let request: Request = serde_json::from_str(message)
            .map_err(|e| crate::errors::CompileError::Generic(format!("Failed to parse request: {}", e)))?;
        
        // Handle request
        if let Some(id) = request.id {
            // Request with ID - needs response
            let response = self.handle_request(id, &request.method, request.params)?;
            Ok(Some(serde_json::to_string(&response).map_err(|e| crate::errors::CompileError::Generic(e.to_string()))?))
        } else {
    ...
    fn handle_request(&mut self, id: Value, method: &str, params: Option<Value>) -> Result<Response> {
    ...
    fn handle_initialize(&mut self, params: Option<Value>) -> std::result::Result<Value, ResponseError> {
        #[derive(Deserialize)]
        ...
        // Initialize server
        self.server.lock().unwrap().initialize(params.root_uri)
    ...
    fn handle_did_open(&mut self, params: Option<Value>) -> Result<()> {
        ...
        if let Some(params) = params {
            let params: DidOpenParams = serde_json::from_value(params).map_err(|e| crate::errors::CompileError::Generic(e.to_string()))?;
    ...
    fn handle_did_change(&mut self, params: Option<Value>) -> Result<()> {
        ...
        if let Some(params) = params {
            let params: DidChangeParams = serde_json::from_value(params).map_err(|e| crate::errors::CompileError::Generic(e.to_string()))?;
    ...
    fn handle_did_close(&mut self, params: Option<Value>) -> Result<()> {
        ...
        if let Some(params) = params {
            let params: DidCloseParams = serde_json::from_value(params).map_err(|e| crate::errors::CompileError::Generic(e.to_string()))?;
    ...
    fn send_diagnostics(&mut self, uri: &str) -> Result<()> {
        let diagnostics = self.server.lock().unwrap()
            .diagnostics
            .get(uri)
            .cloned()
            .unwrap_or_default();
        ...
        let message = serde_json::to_string(&notification).map_err(|e| crate::errors::CompileError::Generic(e.to_string()))?;
    ...
    fn handle_completion(&self, params: Option<Value>) -> std::result::Result<Value, ResponseError> {
        #[derive(Deserialize)]
        ...
        if let Some(params) = params {
            ...
            let completions = self.server.lock().unwrap()
