    ...
    pub fn build_package(&mut self, package_name: &str) -> Result<PathBuf> {
        ...
        if manifest.bin.is_empty() {
            if let Some(main_path) = &manifest.main {
                let artifact =
                    self.build_binary(&manifest, main_path, &manifest.name, &output_dir)?;
                built_artifacts.push(artifact);
            } else if Path::new("src/main.pd").exists() {
                let artifact =
                    self.build_binary(&manifest, "src/main.pd", &manifest.name, &output_dir)?;
                built_artifacts.push(artifact);
            }
        }
        ...
        if built_artifacts.is_empty() {
            return Err(CompileError::Generic(format!(
                "No build targets found for package '{}'",
                package_name
            )));
        }
...
impl BuildSystem {
    ...
    pub fn build(&mut self) -> Result<()> {
        ...
        // Load root package
        let _root_package = self.context.load_package(Path::new("package.pd"))?;
    ...
    pub fn run(&mut self, args: Vec<String>) -> Result<()> {
        ...
        let exe_dir = self
            ...
            .config
            .target_dir
            .join(if self.context.config.release {
                "release"
            } else {
                "debug"
            })
    ...
    fn run_test(&mut self, test_path: &Path) -> Result<()> {
        ...
        let output_path = self
            .context
            .compile_file(test_path, test_name, &output_dir, false)?;
