...
impl BootstrapCompiler {
    ...
    pub fn new() -> Result<Self> {
        ...
        Ok(Self {
            compiler_path: "bootstrap/v3_incremental/archive/versioned_compilers/tiny_v16_compiler".to_string(),
    ...
    fn build_bootstrap_compiler() -> Result<Self> {
        ...
        let source_path = Path::new("bootstrap/v3_incremental/archive/versioned_compilers/tiny_v16.pd");
        let output_c = Path::new("build_output/tiny_v16.c");
        let output_exe = Path::new("bootstrap/v3_incremental/archive/versioned_compilers/tiny_v16_compiler");
    ...
    pub fn compile(&self, source_path: &Path) -> Result<()> {
        println!("🚀 Using bootstrap compiler {} to compile {}", 
                 self.version, source_path.display());
        ...
        let mut child = Command::new(&self.compiler_path)
            ...
            .map_err(|e| CompileError::Generic(format!(
                "Failed to run bootstrap compiler: {}", e
            )))?;
        ...
        fs::write(&c_output_path, &output.stdout)
            .map_err(|e| CompileError::IoError(e))?;
...
pub fn validate_bootstrap(source_path: &Path) -> Result<()> {
    ...
    let driver = crate::Driver::new();
    let rust_output = driver.compile_file(source_path)?;
...
pub fn self_hosting_test() -> Result<()> {
    ...
    let bootstrap_source = Path::new("bootstrap/v3_incremental/archive/versioned_compilers/tiny_v16.pd");
    
    println!("📝 Attempting to compile {} with itself...", bootstrap_source.display());
