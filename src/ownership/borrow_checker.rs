...
impl BorrowChecker {
    ...
    /// emits every public imported struct (`src/codegen/mod.rs:1360-1368`) and then
    /// every local struct (`src/codegen/mod.rs:1384-1388`) with no shadowing check
            ...
            for item in &module_info.ast.items {
                match item {
                    Item::Struct(struct_def) => {
                        ...
                        // imported structs (`src/codegen/mod.rs:1363-1364`), so a
    ...
    pub fn check_program(&mut self, program: &Program) -> Result<()> {
        ...
        // `Item::Enum` (`src/codegen/mod.rs:1359-1376`) and, separately,
        // `Item::Function` (`src/codegen/mod.rs:1464-1464`) — there is no `Item::Impl`
        ...
        for module_name in module_names {
            for item in &modules[module_name].ast.items {
                if let Item::Function(func) = item {
                    ...
                    // (`src/codegen/mod.rs:1466-1468`, public and non-generic) and
