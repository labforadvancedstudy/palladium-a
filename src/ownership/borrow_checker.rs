...
impl BorrowChecker {
    ...
    /// emits every public imported struct (`src/codegen/mod.rs:1365-1373`) and then
    /// every local struct (`src/codegen/mod.rs:1389-1393`) with no shadowing check
            ...
            for item in &module_info.ast.items {
                match item {
                    Item::Struct(struct_def) => {
                        ...
                        // imported structs (`src/codegen/mod.rs:1368-1369`), so a
    ...
    pub fn check_program(&mut self, program: &Program) -> Result<()> {
        ...
        // `Item::Enum` (`src/codegen/mod.rs:1364-1381`) and, separately,
        // `Item::Function` (`src/codegen/mod.rs:1469-1469`) — there is no `Item::Impl`
        ...
        for module_name in module_names {
            for item in &modules[module_name].ast.items {
                if let Item::Function(func) = item {
                    ...
                    // (`src/codegen/mod.rs:1471-1473`, public and non-generic) and
