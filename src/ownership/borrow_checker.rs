...
impl BorrowChecker {
    ...
    /// (`src/typeck/mod.rs:1219-1219`), because the driver has one resolver result and
    ...
    /// filter (`src/typeck/mod.rs:1399-1400`), so the two passes agree on which
    /// `P` is meant.
    ///
    ...
    /// `crate::ast::local_type_shadows_import` (`src/codegen/mod.rs:1595-1623`)
            ...
            for item in &module_info.ast.items {
                match item {
                    Item::Struct(struct_def) => {
                        ...
                        // imported structs (`src/codegen/mod.rs:1473-1474`), so a
    ...
    pub fn check_program(&mut self, program: &Program) -> Result<()> {
        ...
        // `Item::Enum` (`src/codegen/mod.rs:1595-1623`) and, separately,
        // `Item::Function` (`src/codegen/mod.rs:1737-1738`) — there is no `Item::Impl`
        ...
        for module_name in module_names {
            for item in &modules[module_name].ast.items {
                if let Item::Function(func) = item {
                    ...
                    // (`src/codegen/mod.rs:1738-1741`, public and non-generic) and
