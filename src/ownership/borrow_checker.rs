...
impl BorrowChecker {
    ...
    /// (`src/typeck/mod.rs:601-601`), because the driver has one resolver result and
    ...
    /// emits every public imported struct (`src/codegen/mod.rs:1385-1393`) and then
    /// every local struct (`src/codegen/mod.rs:1409-1413`) with no shadowing check
            ...
            for item in &module_info.ast.items {
                match item {
                    Item::Struct(struct_def) => {
                        ...
                        // imported structs (`src/codegen/mod.rs:1388-1389`), so a
    ...
    pub fn check_program(&mut self, program: &Program) -> Result<()> {
        ...
        // `Item::Enum` (`src/codegen/mod.rs:1384-1401`) and, separately,
        // `Item::Function` (`src/codegen/mod.rs:1489-1489`) — there is no `Item::Impl`
        ...
        for module_name in module_names {
            for item in &modules[module_name].ast.items {
                if let Item::Function(func) = item {
                    ...
                    // (`src/codegen/mod.rs:1491-1493`, public and non-generic) and
