...
impl BorrowChecker {
    ...
    /// (`src/typeck/mod.rs:1457-1457`), because the driver has one resolver result and
    ...
    /// filter (`src/typeck/mod.rs:1637-1638`), so the two passes agree on which
    /// `P` is meant.
    ///
    ...
    fn is_copy_type(&self, ty: &Type) -> bool {
        match ty {
            // `char` is a scalar in a `long long`, so it copies like every other
            // scalar here — N4-04 changed what it MEANS, not what it costs.
            Type::I32
            | Type::I64
            | Type::U32
            | Type::U64
