...
impl BorrowChecker {
    ...
    /// filter (`src/typeck/mod.rs:553-553`), so the two passes agree on which
    /// `P` is meant.
    ///
    ...
    /// emits every public imported struct (`src/codegen/mod.rs:1305-1322`) and then
    /// every local struct (`src/codegen/mod.rs:1305-1322`) with no shadowing check
            ...
            for item in &module_info.ast.items {
                match item {
                    Item::Struct(struct_def) => {
                        ...
                        // imported structs (`src/codegen/mod.rs:1305-1322`), so a
    ...
    pub fn check_program(&mut self, program: &Program) -> Result<()> {
        ...
        // `Item::Function` (`src/codegen/mod.rs:1305-1322`) — there is no `Item::Impl`
        ...
        for module_name in module_names {
            for item in &modules[module_name].ast.items {
                if let Item::Function(func) = item {
                    ...
                    // (`src/codegen/mod.rs:1305-1322`, public and non-generic) and
