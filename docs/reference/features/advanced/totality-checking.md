...
types with pattern matching on subterms. `match` HAS the pattern forms now (`src/ast/mod.rs:565`:
literal, range, tuple, or- and `@` patterns, with guards), so the matching half is no longer the
blocker. Generic types still do not survive codegen (`src/codegen/mod.rs:2741-2741`), and there is
