...
the compiler — `src/driver/mod.rs:157` — and all the driver does with the result is `println!` it
(`src/driver/mod.rs:161`). No later phase reads it. It cannot reject a program, cannot change
...
`crate::ast::Item::Function` (`src/driver/mod.rs:158-159`), so functions inside `impl` blocks are
not analysed at all.
...
(`src/codegen/mod.rs:2621-2628`) and then reads `<tmp>.result` (`src/codegen/mod.rs:2630-2632`). Nothing generates a
...
recorded. The parallel defect for `?` is at `src/codegen/mod.rs:2565-2586`, which emits a
