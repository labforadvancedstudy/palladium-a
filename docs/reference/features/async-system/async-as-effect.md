...
(`src/codegen/mod.rs:2659-2666`) and then reads `<tmp>.result` (`src/codegen/mod.rs:2668-2670`). Nothing generates a
...
recorded. The parallel defect for `?` is at `src/codegen/mod.rs:2603-2624`, which emits a
