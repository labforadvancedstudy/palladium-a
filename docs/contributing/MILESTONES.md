...
`.values()`, and generic instantiations in `HashMap`s (`src/typeck/mod.rs:1120-1120`,
...
builtin registry already classifies every builtin (`src/builtins.rs:200`), the analyser already
...
enum-owned method is unreachable by its path form. Each row carries a
   ...
   *What this item deliberately does NOT include:* **N13-03** (arguments are evaluated left to
   right) was untouched by THIS item and stayed `owed` after it. *(It landed later, in
   `feat/m2-types-semantics`, as an obligation over every call rather than a method-call one —
   which is exactly what the next sentence predicted. Recorded here rather than rewritten, because
   what this item did not do is still what it did not do.)* Method calls make the receiver the first argument and
...
M4 owns (`tests/conformance-manifest.txt:127`, cross-file imports; the other five are su3's
...
122 of them: `reject=122` over 221 fixtures (the char-pattern round added the last three, all boundaries of one new surface: a range with one `char` end and one integer end, a char pattern against an `i64` scrutinee, and an empty char range; su3 added the three before them: a chained receiver `self.d.bump()`, and the two halves of the `let` production — a missing initialiser and a destructuring binding — which the specification had refused in prose since it was written and which nothing had ever executed; the gpt56 review round added the one before them, a reference parameter handed a temporary — front-end approval plus a gcc rejection, which pre-dated su2 and which su2's call-site predicate now owns; the su2 review round added the two before it, both the CALL path: a `&self` method and a by-value-`self` method each calling a `&mut self` method on `self`, which is a write through a receiver that promised not to be one; su2 itself added the four before them, every one a write the type checker now refuses: a field write through a shared `&self` receiver, the same write through a by-value `self` — which used to mutate a copy and print the stale value — `self` itself as an assignment target, and `*self` as a place; N6 field shorthand added the six before them — the outer boundary, a brace pattern with no variant path; the inner one, the brace form on a TUPLE variant, which the type checker rather than the parser refuses; and the two halves of the subset rule, an unknown field and reading an omitted one, which a run fixture cannot witness because both are refusals; and the two PARITY fixtures executing the explicit spelling of the unknown-field and tuple-variant refusals; N4-04 added the eight before them: one per direction of the char/i64 split, the binary-operator branch, the four forbidden cast cells, and an `as char` whose operand is not a Unicode scalar; N14-02 added the three before them — defining a built-in name, shadowing one with a local binder, and taking one for a type, the third carrying a different reason from the other two. It was 92 over 183 until then, the denominator having moved alone once because N13-03's fixture is a `run` row; and 89 over 178 until `feat/m2-types-semantics` added three — the `for`-over-a-nested-array refusal and the two inner-length declarator positions; and was 87 over 176 until the round-3 review of `feat/m2-items` added two — the two `<<` branches the count-range fixture never covered, a legal AMOUNT with an illegal VALUE and a negative left operand; and was 82 over 171 until the round that followed added five more — the branches of the one-namespace and `pub` rules that the first round stated in prose and pinned in only one spelling each; and was 69 over 158 until the review round of `feat/m2-items` added THIRTEEN, which is what a review round costs when the reviewers probe rather than read: two fresh-binder shadowing shapes, both directions of a cross-kind name collision, four initialisers with no value, `pub` on an item, a macro invocation in an argument, two literal-kind losses and the macro_rules invocation spelling; and was 63 over 151 until the macro round of `feat/m2-items`
