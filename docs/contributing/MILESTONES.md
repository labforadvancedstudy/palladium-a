...
| No missing-return diagnostic — `fn f() -> int { }` compiled silently. **CLOSED**: the parser already decided "returns on every path" and now refuses when it does not, `src/parser/mod.rs:1256-1286`; the `#[ignore]` is gone and the debt row is `paid` | `tests/compiler_comprehensive_test.rs:633` | N3-03 |
