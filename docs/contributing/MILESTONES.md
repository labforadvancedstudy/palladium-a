...
sites sorts before it emits: the imported-module walks at `src/codegen/mod.rs:2055-2056` and
`src/typeck/mod.rs:1490-1490`, the two later codegen walks off their own sorted locals
(`src/codegen/mod.rs:2141-2141`, `src/codegen/mod.rs:3107-3107`), and the instantiation keys at
`src/typeck/mod.rs:6180-6181` and `src/typeck/mod.rs:6242-6243`. Pinned by
...
reverting either guard alone now fails a fixture of its own; then `feat/m2-types-semantics` added eight — two `run` fixtures, `tests/02_types_nested_arrays.pd` (N4-10) and `tests/03_arg_evaluation_order.pd` (N13-03, call arguments read left to right), and six `reject`s: `for_over_nested_array.pd`, the two inner-length declarator positions (parameter and struct field), N14-02's three built-in-name positions — a definition, a local binder and a type, which are refused for two DIFFERENT reasons and so need three rows rather than one — and N4-04's two, one per DIRECTION of the char/i64 split, because a one-way widening would have left `char` an `i64` wearing a hat) | `make conformance` |
...
M1 fixed the `.await` **consumer** — `src/codegen/mod.rs:6401-6405` returns
...
`check_function`) and again at the defect in code generation (`src/codegen/mod.rs:3406-3412`), the
