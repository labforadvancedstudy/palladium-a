...
`.values()`, and generic instantiations in `HashMap`s (`src/typeck/mod.rs:1100-1100`,
`src/typeck/mod.rs:1108-1108`) emitted by iterating `.keys()`.

**Both are now ordered, and this paragraph was written before they were.** Every one of the four
sites sorts before it emits: the imported-module walks at `src/codegen/mod.rs:2074-2075` and
`src/typeck/mod.rs:1490-1490`, the two later codegen walks off their own sorted locals
(`src/codegen/mod.rs:2160-2160`, `src/codegen/mod.rs:3129-3129`), and the instantiation keys at
`src/typeck/mod.rs:6218-6219` and `src/typeck/mod.rs:6280-6281`. Pinned by
...
(`tests/conformance-manifest.txt:125`, cross-file imports) and the vacuous `12_modules_imports`.
...
M1 fixed the `.await` **consumer** — `src/codegen/mod.rs:6428-6432` returns
...
`check_function`) and again at the defect in code generation (`src/codegen/mod.rs:3428-3434`), the
