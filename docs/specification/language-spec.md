...
consumed only for import aliases (`src/parser/mod.rs:637`).
...
(`src/parser/mod.rs:554`), so an `import` after a `fn` is a syntax error.
...
Items (`src/parser/mod.rs:722`): `fn`, `struct`, `enum`, `trait`, `impl`, `type`, `macro`.
...
`Function.effects` is hardcoded `None` by the parser (`src/parser/mod.rs:969`, corrected from
v0.2's `src/parser/mod.rs:953`, which is where the `Function` literal opens). Effects are *inferred* afterwards
...
field (`src/parser/mod.rs:757`, `src/ast/mod.rs:139`).

### A4.4 Traits

**unimplemented.** Traits parse (`src/parser/mod.rs:1156`, corrected from line 736–960 of the pre-cleanup revision) and then
...
(`src/parser/mod.rs:1280`, corrected from line 863–897 of the pre-cleanup revision).
...
contain only `fn` (`src/parser/mod.rs:1449-1455`, corrected from line 1030 of the pre-cleanup revision).
...
(`src/parser/mod.rs:1678`, corrected from line 1258 of the pre-cleanup revision).
...
| Syntax | Status | Note |
|---|---|---|
| `i64`, `int` | implemented | `int` is an alias for `i64` (`src/parser/mod.rs:2530`, corrected from line 2038 of the pre-cleanup revision) |
| `i32`, `u32`, `u64` | implemented | primitive table at `src/parser/mod.rs:2528-2536` (corrected from line 2037–2043 of the pre-cleanup revision) |
...
uppercase or `_` is reclassified as a *const generic argument* (`src/parser/mod.rs:2566-2576`,
...
`unsafe { }`, expression statements (`src/parser/mod.rs:1751`).

- implemented: `let [mut] x [: T] = e;` — **the initializer is mandatory**
  (`src/parser/mod.rs:1835`, corrected from line 1411 of the pre-cleanup revision); the binding must be a plain identifier
  (no patterns).
- implemented: assignment targets — identifier, index, field, deref.
- **unimplemented: `else if`** — after `else` the parser requires `{`
  (`src/parser/mod.rs:1909`, corrected from line 1441 of the pre-cleanup revision). Verified: `if a {} else if b {}` →
...
- **unimplemented: `if`, `match`, and blocks are statements, not expressions**
  (`src/parser/mod.rs:1755`, `src/parser/mod.rs:1760`). `let x = if c { 1 } else { 2 };` does not parse. This is a
...
its right operand (`src/parser/mod.rs:2456`, corrected from line 1964 of the pre-cleanup revision), so `a * -b` fails to parse.
...
`Function.lifetime_params` is parsed (`src/parser/mod.rs:957`) and read nowhere outside test and
