...
consumed only for import aliases (`src/parser/mod.rs:638`).
...
(`src/parser/mod.rs:555`), so an `import` after a `fn` is a syntax error.
...
Items (`src/parser/mod.rs:723`): `fn`, `struct`, `enum`, `trait`, `impl`, `type`, `macro`.
...
`Function.effects` is hardcoded `None` by the parser (`src/parser/mod.rs:970`, corrected from
v0.2's `src/parser/mod.rs:954`, which is where the `Function` literal opens). Effects are *inferred* afterwards
...
field (`src/parser/mod.rs:758`, `src/ast/mod.rs:139`).

### A4.4 Traits

**unimplemented.** Traits parse (`src/parser/mod.rs:1157`, corrected from line 736–960 of the pre-cleanup revision) and then
...
(`src/parser/mod.rs:1281`, corrected from line 863–897 of the pre-cleanup revision).
...
contain only `fn` (`src/parser/mod.rs:1450-1456`, corrected from line 1030 of the pre-cleanup revision).
...
(`src/parser/mod.rs:1679`, corrected from line 1258 of the pre-cleanup revision).
...
| Syntax | Status | Note |
|---|---|---|
| `i64`, `int` | implemented | `int` is an alias for `i64` (`src/parser/mod.rs:2531`, corrected from line 2038 of the pre-cleanup revision) |
| `i32`, `u32`, `u64` | implemented | primitive table at `src/parser/mod.rs:2529-2537` (corrected from line 2037–2043 of the pre-cleanup revision) |
...
uppercase or `_` is reclassified as a *const generic argument* (`src/parser/mod.rs:2567-2577`,
...
`unsafe { }`, expression statements (`src/parser/mod.rs:1752`).

- implemented: `let [mut] x [: T] = e;` — **the initializer is mandatory**
  (`src/parser/mod.rs:1836`, corrected from line 1411 of the pre-cleanup revision); the binding must be a plain identifier
  (no patterns).
- implemented: assignment targets — identifier, index, field, deref.
- **unimplemented: `else if`** — after `else` the parser requires `{`
  (`src/parser/mod.rs:1910`, corrected from line 1441 of the pre-cleanup revision). Verified: `if a {} else if b {}` →
...
- **unimplemented: `if`, `match`, and blocks are statements, not expressions**
  (`src/parser/mod.rs:1756`, `src/parser/mod.rs:1761`). `let x = if c { 1 } else { 2 };` does not parse. This is a
...
its right operand (`src/parser/mod.rs:2457`, corrected from line 1964 of the pre-cleanup revision), so `a * -b` fails to parse.
...
`Function.lifetime_params` is parsed (`src/parser/mod.rs:958`) and read nowhere outside test and
