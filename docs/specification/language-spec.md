...
- generic → "Generic types in structs not yet supported" (`src/codegen/mod.rs:1877-1877`)
- reference → "Reference types in structs not yet supported" (`src/codegen/mod.rs:1586-1586`)
- tuple → "Tuple types in structs not yet supported" (`src/codegen/mod.rs:1890-1894`)
...
emit nothing — codegen ignores `Item::Trait` (`src/codegen/mod.rs:1517-1520`, corrected from line 754–757 of the pre-cleanup revision). Trait method bodies are never typechecked (`src/typeck/mod.rs:1107-1109`, corrected
...
(`src/codegen/mod.rs:1199-1205`, corrected from line 1861 of the pre-cleanup revision).
...
| `(A, B)` | partial | becomes `void*` in C (`src/codegen/mod.rs:1257-1260`, corrected from line 828 of the pre-cleanup revision); no tuple expression exists, so no tuple is constructible |
...
(`src/codegen/mod.rs:1229-1231`). Neither is monomorphised. *(v0.2 said "array sizes from a const
...
- partial: ranges outside a `for` header — codegen error "Range expressions can only be used in
  for loops" (`src/codegen/mod.rs:2739-2742`, corrected from line 2121 of the pre-cleanup revision).
...
generic enum definitions (`src/codegen/mod.rs:2820-2841`, `src/codegen/mod.rs:2876-2887`, `src/codegen/mod.rs:1013-1013`) and generic enum construction
...
`src/typeck/mod.rs:2974-2974`) and again by code generation (`?` at `src/codegen/mod.rs:3317-3321`,
...
- `.await` emitted `while (!f.poll(&f)) {}`. C has no member function calls, and the poll
  ...
  (`src/codegen/mod.rs:2862-2862`), which that call never names. There is no async runtime.
...
(`src/codegen/mod.rs:2198-2198`, `src/codegen/mod.rs:2219-2230`) with a wildcard arm becoming the final `else`; when no
...
(`src/codegen/mod.rs:2443-2443`, corrected from line 1813–1851 of the pre-cleanup revision) and emits their C bodies inline into
