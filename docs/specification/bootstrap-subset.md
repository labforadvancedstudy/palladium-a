...
- Tuples — `type_to_c` yields `void*` (`src/codegen/mod.rs:1758-1761`), and a tuple in a struct
  field is a hard error (`src/codegen/mod.rs:2192-2192`). No tuple *expressions* exist at all, so no
  tuple is constructible.
- Generic types in struct fields — error at `src/codegen/mod.rs:2177-2177`.
- Reference types in struct fields — error at `src/codegen/mod.rs:2182-2182`.
- Returning an array from a function — error at `src/codegen/mod.rs:2415-2419`.
- `char`, `str`, `u8`, `usize` — no such primitives; `src/parser/mod.rs:2836-2845` is the whole
  set the type parser recognises. `f32`/`f64` were in this bullet and no longer belong: M2 added
  them (`src/parser/mod.rs:2840-2841`, requirement N4-02), so they stay out of PBS-1 by CHOICE,
...
- `Option<T>` / `Result<T,E>` as built-ins — they do not exist. Declaring your own does not
  ...
  is rejected outright (`src/typeck/mod.rs:3729-3729`). It used to emit a C `struct Result` layout
...
(`src/parser/mod.rs:2818-2845`), so `Foo<T>` does not mean what it looks like.
...
3. **Struct and array parameters that are written are declared `mut`.**
   ...
   (`src/ownership/borrow_checker.rs:350-356`) and can never be used again by the caller.
...
- **`let` requires an initializer.** `let x: i64;` is a parse error (`src/parser/mod.rs:959`).
- **No `else if`.** After `else` the parser demands `{` (`src/parser/mod.rs:1845`). Write
...
| Construct | Why |
|---|---|
| `if` / `match` / block as an *expression* | parsed only as statements (`src/parser/mod.rs:2026`, `src/parser/mod.rs:2032`) |
| method call `x.f()` | typeck rejects: "Indirect function calls not yet supported" (`src/typeck/mod.rs:2886-2891`). Call `Type::method(receiver, …)` instead. |
| `?` operator | rejected: "the `?` operator is not implemented" (`src/typeck/mod.rs:3729-3729`). It used to emit C referencing an undefined `struct Result`. |
| `.await` / `async` | `.await` rejected: "`.await` is not implemented" (`src/typeck/mod.rs:3736-3736`). It used to emit a `poll` member call that is never generated. |
| closures | no closure token path, no closure AST node |
| ranges outside `for` | codegen error (`src/codegen/mod.rs:2678-2678`) |
| empty array literal `[]` | typeck error — element type uninferrable (`src/typeck/mod.rs:3203-3203`) |
...
Exactly three forms exist (`src/ast/mod.rs:357-367`):
...
| D2 | 11 builtins registered in typeck but not in the borrow checker, so `string_len`, `string_eq`, `string_char_at`, `string_from_char`, `char_is_digit/alpha/whitespace`, `file_read_all`, `file_read_line`, `file_write` and `panic` failed with `Use of uninitialized value` | `src/ownership/borrow_checker.rs` vs `src/typeck/mod.rs:985-991` | **fixed** — `src/builtins.rs` is now the single table both passes derive from, with drift tests |
| D3 | a tail expression in a value-returning function emitted no `return`, so `fn add(a,b) -> i64 { a + b }` compiled clean and returned garbage. All of `stdlib/` was affected | `src/parser/mod.rs:456`, `src/codegen/mod.rs:2743-2746` | **fixed** — lowered to `Stmt::Return` in the parser |
...
| D5 | `?` emitted C for a `struct Result` layout codegen never defines, and `.await` emitted a call to a `poll` member no generated struct has. Neither was an error: both programs died inside gcc, against C the user never wrote. The LLVM backend was worse — its catch-all returns the constant `0` for both | `src/codegen/mod.rs:2717-2717`, `src/codegen/mod.rs:2780-2780` (pre-fix); `src/codegen/llvm_text_backend.rs:1385` | **fixed** — both rejected with "is not implemented" plus consequence and a workaround that is compiled and run by `tests/d5_unimplemented_constructs.rs` (`src/typeck/mod.rs:3729-3729`, `src/typeck/mod.rs:3736-3736`; backstop at `src/codegen/mod.rs:3647-3647`, `src/codegen/mod.rs:3659-3659`). Old lowerings deleted, not flagged off. PBS-1 still excludes both |
