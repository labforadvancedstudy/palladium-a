...
- Reference types in struct fields — error at `src/codegen/mod.rs:1372`.
- Returning an array from a function — error at `src/codegen/mod.rs:1521` ("Returning arrays from functions is not yet supported").
...
- **`for` iterates a range or an array only.** Iterating an array *parameter* miscompiles:
  codegen emits `sizeof(arr)/sizeof(arr[0])` (`src/codegen/mod.rs:1920-1922`), which is wrong
