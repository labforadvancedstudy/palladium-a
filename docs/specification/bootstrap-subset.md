...
- `Option<T>` / `Result<T,E>` as built-ins — they do not exist. Declaring your own does not
  ...
  is rejected outright (`src/typeck/mod.rs:4976-4976`). It used to emit a C `struct Result` layout
...
| `?` operator | rejected: "the `?` operator is not implemented" (`src/typeck/mod.rs:4976-4976`). It used to emit C referencing an undefined `struct Result`. |
