...
- Tuples — `type_to_c` yields `void*` (`src/codegen/mod.rs:828`), and a tuple in a struct
  field is a hard error (`:1119`). No tuple *expressions* exist at all, so no tuple is
  constructible.
- Generic types in struct fields — error at `src/codegen/mod.rs:1104`.
...
- `Option<T>` / `Result<T,E>` as built-ins — they do not exist. `?` typechecks against a
  `Result` shape but codegen emits a C `struct Result` layout that no other part of codegen
  ever defines (`src/codegen/mod.rs:2160-2201`), so `?` produces uncompilable C.
...
| `?` operator | emits C referencing an undefined `struct Result` (`src/codegen/mod.rs:2160`) |
| `.await` / `async` | emits a `poll` call that is never generated (`src/codegen/mod.rs:2208`) |
