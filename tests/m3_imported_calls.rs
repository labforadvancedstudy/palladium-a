...
// (`src/codegen/mod.rs:1575-1575`) and so a skipped body "produces no C". That
...
// (`docs/contributing/MILESTONES.md:793-799`, "M4 — Modules", which claims the
...
// (`docs/contributing/MILESTONES.md:759`), which is not what these rows are
// about.
//
...
/// `place_type` (`src/ownership/borrow_checker.rs:1378`) consults to decide
...
/// `src/resolver/mod.rs:195` is `let _sub_modules = self.resolve_program(&ast)?;`,
...
#[test]
#[ignore = "XFAIL: use-after-move is accepted when the binding has no type annotation — BorrowChecker::expr_type returns Type::I64 for every non-literal (the `Expr::Ident(_) => Type::I64, // TODO: Proper type lookup` arm), and Stmt::Let consults it only for an unannotated binding, so `let a = S { v: 1 }; let b = a; let c = a;` classifies the struct as Copy and compiles, while the byte-identical program with `: S` on each binding is refused with \"Use of moved value: a\"; reproduces on main (owned by M4, the type model the ownership pass reads)"]
...
/// re-derives visibility from `ast.items` instead: `src/typeck/mod.rs:1439-1439`,
/// `src/codegen/mod.rs:1575-1575`, `src/codegen/mod.rs:1680-1680`,
/// `src/codegen/mod.rs:1808-1808`, `src/codegen/mod.rs:2587-2587`, and the borrow checker's
...
/// imported function (`src/codegen/mod.rs:1951-1961`) and emits the local one
/// unconditionally (`src/codegen/mod.rs:1968-1974`). This test is green.
///
...
/// `src/codegen/mod.rs:1957-1957`, and BOTH of the citations it leaned on had
...
/// A qualified call cannot be written. `src/parser/mod.rs:3824-3865` turns any
/// `a::b(...)` into `Expr::EnumConstructor`, and `src/typeck/mod.rs:3987-3991`
...
#[test]
#[ignore = "XFAIL: a qualified call `lib2::helper()` is unreachable — src/parser/mod.rs:3824-3865 turns every `a::b(...)` into Expr::EnumConstructor and src/typeck/mod.rs:3987-3991 then reports \"Undefined enum type: lib2\"; the same makes `import lib2 as m;` unusable, since `m::helper()` reports \"Undefined enum type: m\" (owned by M4, cross-file module imports)"]
...
/// resolver. `src/parser/mod.rs:802-813`: after `::`, if the token after the
...
/// (`src/codegen/mod.rs:4412-4415`), while the type checker
...
/// (`src/codegen/mod.rs:1811-1839`) and imported function bodies
/// (`src/codegen/mod.rs:1951-1963`) — from the prototype block
/// (`src/codegen/mod.rs:2584-2595`), which is a fourth site and emits
...
/// iterating `self.instantiations.keys()` (`src/typeck/mod.rs:5202-5202`), which is a
/// `HashMap`, and `get_struct_instantiations` does the same
/// (`src/typeck/mod.rs:5264-5264`). Codegen then emits in that Vec's order
/// (`src/codegen/mod.rs:1758-1758`, `src/codegen/mod.rs:2599-2599`).
///
...
/// (excluded from PBS-1, `docs/specification/bootstrap-subset.md:94`). Today's
...
/// (`src/typeck/mod.rs:5264-5264`). Generic *structs* cannot be compiled at all right
