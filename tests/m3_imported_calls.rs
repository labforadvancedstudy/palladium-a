...
// (`src/codegen/mod.rs:1930-1930`) and so a skipped body "produces no C". That
...
/// `src/codegen/mod.rs:1930-1930`, `src/codegen/mod.rs:1853-1853`,
/// `src/codegen/mod.rs:2163-2163`, `src/codegen/mod.rs:3113-3113`, and the borrow checker's
...
/// imported function (`src/codegen/mod.rs:2324-2334`) and emits the local one
/// unconditionally (`src/codegen/mod.rs:2341-2347`). This test is green.
///
...
/// `src/codegen/mod.rs:2330-2330`, and BOTH of the citations it leaned on had
...
/// `a::b(...)` into `Expr::EnumConstructor`, and `src/typeck/mod.rs:4610-4614`
...
#[test]
#[ignore = "XFAIL: a qualified call `lib2::helper()` is unreachable — src/parser/mod.rs:4393-4434 turns every `a::b(...)` into Expr::EnumConstructor and src/typeck/mod.rs:4610-4614 then reports \"Undefined enum type: lib2\"; the same makes `import lib2 as m;` unusable, since `m::helper()` reports \"Undefined enum type: m\" (owned by M4, cross-file module imports)"]
...
/// (`src/codegen/mod.rs:5913-5915`), while the type checker
...
/// `long long`.
#[test]
#[ignore = "XFAIL: an imported name that shadows a built-in is registered by the checkers but ignored by codegen — src/typeck/mod.rs:1604-1604 and the borrow checker insert the imported signature over the built-in, while src/codegen/mod.rs:5913-5915 tests is_builtin() first and unconditionally, so `pub fn print_int(s: String)` type-checks `print_int(\"hello\")` and then emits `__pd_print_int(\"hello\")`, which gcc rejects as \"incompatible pointer to integer conversion\" (owned by M4, cross-file module imports)"]
...
/// (`src/codegen/mod.rs:2166-2194`) and imported function bodies
/// (`src/codegen/mod.rs:2324-2336`) — from the prototype block
/// (`src/codegen/mod.rs:3110-3121`), which is a fourth site and emits
...
/// iterating `self.instantiations.keys()` (`src/typeck/mod.rs:6180-6180`), which is a
/// `HashMap`, and `get_struct_instantiations` does the same
/// (`src/typeck/mod.rs:6242-6242`). Codegen then emits in that Vec's order
/// (`src/codegen/mod.rs:2113-2113`, `src/codegen/mod.rs:2294-2294`).
///
...
/// (excluded from PBS-1, `docs/specification/bootstrap-subset.md:110`). Today's
...
/// (`src/typeck/mod.rs:6242-6242`). Generic *structs* cannot be compiled at all right
