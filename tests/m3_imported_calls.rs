...
// (`src/codegen/mod.rs:1758-1758`) and so a skipped body "produces no C". That
...
// (`docs/contributing/MILESTONES.md:868-874`, "M4 — Modules", which claims the
...
// (`docs/contributing/MILESTONES.md:834`), which is not what these rows are
// about.
//
...
/// `place_type` (`src/ownership/borrow_checker.rs:1535-1537`) consults to decide
...
/// `src/codegen/mod.rs:1758-1758`, `src/codegen/mod.rs:1681-1681`,
/// `src/codegen/mod.rs:1991-1991`, `src/codegen/mod.rs:2824-2824`, and the borrow checker's
...
/// imported function (`src/codegen/mod.rs:2146-2156`) and emits the local one
/// unconditionally (`src/codegen/mod.rs:2163-2169`). This test is green.
///
...
/// `src/codegen/mod.rs:2152-2152`, and BOTH of the citations it leaned on had
...
/// A qualified call cannot be written. `src/parser/mod.rs:4104-4145` turns any
/// `a::b(...)` into `Expr::EnumConstructor`, and `src/typeck/mod.rs:4056-4060`
...
#[test]
#[ignore = "XFAIL: a qualified call `lib2::helper()` is unreachable — src/parser/mod.rs:4104-4145 turns every `a::b(...)` into Expr::EnumConstructor and src/typeck/mod.rs:4056-4060 then reports \"Undefined enum type: lib2\"; the same makes `import lib2 as m;` unusable, since `m::helper()` reports \"Undefined enum type: m\" (owned by M4, cross-file module imports)"]
...
/// (`src/codegen/mod.rs:5259-5261`), while the type checker
...
/// (`src/codegen/mod.rs:1994-2022`) and imported function bodies
/// (`src/codegen/mod.rs:2146-2158`) — from the prototype block
/// (`src/codegen/mod.rs:2821-2832`), which is a fourth site and emits
...
/// iterating `self.instantiations.keys()` (`src/typeck/mod.rs:5600-5600`), which is a
/// `HashMap`, and `get_struct_instantiations` does the same
/// (`src/typeck/mod.rs:5662-5662`). Codegen then emits in that Vec's order
/// (`src/codegen/mod.rs:1941-1941`, `src/codegen/mod.rs:2116-2116`).
///
...
/// (`src/typeck/mod.rs:5662-5662`). Generic *structs* cannot be compiled at all right
