...
// (`src/codegen/mod.rs:1949-1949`) and so a skipped body "produces no C". That
...
// (`docs/contributing/MILESTONES.md:1030-1036`, "M4 — Modules", which claims the
...
// (`docs/contributing/MILESTONES.md:996`), which is not what these rows are
// about.
//
...
/// re-derives visibility from `ast.items` instead: `src/typeck/mod.rs:1490-1490`,
/// `src/codegen/mod.rs:1949-1949`, `src/codegen/mod.rs:1872-1872`,
/// `src/codegen/mod.rs:2182-2182`, `src/codegen/mod.rs:3135-3135`, and the borrow checker's
...
/// imported function (`src/codegen/mod.rs:2343-2353`) and emits the local one
/// unconditionally (`src/codegen/mod.rs:2360-2366`). This test is green.
///
...
/// `src/codegen/mod.rs:2349-2349`, and BOTH of the citations it leaned on had
...
/// A qualified call cannot be written. `src/parser/mod.rs:4393-4434` turns any
/// `a::b(...)` into `Expr::EnumConstructor`, and `src/typeck/mod.rs:4611-4615`
...
#[test]
#[ignore = "XFAIL: a qualified call `lib2::helper()` is unreachable — src/parser/mod.rs:4393-4434 turns every `a::b(...)` into Expr::EnumConstructor and src/typeck/mod.rs:4611-4615 then reports \"Undefined enum type: lib2\"; the same makes `import lib2 as m;` unusable, since `m::helper()` reports \"Undefined enum type: m\" (owned by M4, cross-file module imports)"]
...
/// (`src/codegen/mod.rs:5940-5942`), while the type checker
/// (`src/typeck/mod.rs:1604-1604`) and `register_imported_functions` both insert
...
/// `long long`.
#[test]
#[ignore = "XFAIL: an imported name that shadows a built-in is registered by the checkers but ignored by codegen — src/typeck/mod.rs:1604-1604 and the borrow checker insert the imported signature over the built-in, while src/codegen/mod.rs:5940-5942 tests is_builtin() first and unconditionally, so `pub fn print_int(s: String)` type-checks `print_int(\"hello\")` and then emits `__pd_print_int(\"hello\")`, which gcc rejects as \"incompatible pointer to integer conversion\" (owned by M4, cross-file module imports)"]
...
/// (`src/codegen/mod.rs:2185-2213`) and imported function bodies
/// (`src/codegen/mod.rs:2343-2355`) — from the prototype block
/// (`src/codegen/mod.rs:3132-3143`), which is a fourth site and emits
...
/// iterating `self.instantiations.keys()` (`src/typeck/mod.rs:6218-6218`), which is a
/// `HashMap`, and `get_struct_instantiations` does the same
/// (`src/typeck/mod.rs:6280-6280`). Codegen then emits in that Vec's order
/// (`src/codegen/mod.rs:2132-2132`, `src/codegen/mod.rs:2313-2313`).
///
...
/// (excluded from PBS-1, `docs/specification/bootstrap-subset.md:132`). Today's
...
/// (`src/typeck/mod.rs:6280-6280`). Generic *structs* cannot be compiled at all right
...
/// N3-09/N3-10 — A TOP-LEVEL ITEM IN AN IMPORTED MODULE IS REFUSED BY NAME.
///
/// THE MEASUREMENT THIS REPLACES. `const` and `static` items are registered by
/// the type checker's FIRST PASS over the program being compiled, and the
/// import path does not run that pass — it registers imported functions,
/// structs, enums and aliases from `ModuleInfo` and skipped globals entirely.
/// So the module's own body-check (the third pass, which exists precisely so an
/// imported body is not accepted unchecked) reached a name nothing had defined:
///
///     lib.pd:  const LIMIT: i64 = 10;
///              pub fn cap() -> i64 { return LIMIT; }
///     main.pd: import lib;  fn main() { print_int(cap()); }
///
/// failed the IMPORTER'S compile with `Undefined variable or function: 'LIMIT'`
/// — a name out of a file the author of the failing program may never have
/// opened, with no mention of the construct that is missing, and identical
/// whether or not the item said `pub`.
///
/// A REJECT FIXTURE CANNOT HOLD THIS. `scripts/conformance.sh` compiles one
/// `.pd` per row; the defect needs a second module on the search path, so the
/// evidence lives here, in the suite that already has the two-file harness.
#[test]
fn a_top_level_item_in_an_imported_module_is_refused_by_name() {
    for decl in [
        "const LIMIT: i64 = 10;",
        "pub const LIMIT: i64 = 10;",
        "static mut LIMIT: i64 = 10;",
    ] {
        let lib = format!("{}\n\npub fn cap() -> i64 {{\n    return LIMIT;\n}}\n", decl);
        let (compiled, output, _) = compile_and_run(
            &[("lib.pd", lib.as_str())],
            "import lib;\n\nfn main() { print_int(cap()); }\n",
        );
        assert!(!compiled, "`{}` in a module must be refused:\n{}", decl, output);
        assert!(
            output.contains("the imported module `lib` declares the top-level item `LIMIT`"),
            "the refusal must name the MODULE and the ITEM, not the symptom, for `{}`:\n{}",
            decl,
            output
        );
        // The old failure, which must not be what a reader sees any more.
        assert!(
            !output.contains("Undefined variable or function: 'LIMIT'"),
            "`{}` still fails at the use site rather than at the import:\n{}",
            decl,
            output
        );
    }
