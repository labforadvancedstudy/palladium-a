...
// (`src/codegen/mod.rs:1910-1910`) and so a skipped body "produces no C". That
...
/// `src/codegen/mod.rs:1910-1910`, `src/codegen/mod.rs:1833-1833`,
/// `src/codegen/mod.rs:2143-2143`, `src/codegen/mod.rs:3086-3086`, and the borrow checker's
...
/// imported function (`src/codegen/mod.rs:2304-2314`) and emits the local one
/// unconditionally (`src/codegen/mod.rs:2321-2327`). This test is green.
///
...
/// `src/codegen/mod.rs:2310-2310`, and BOTH of the citations it leaned on had
...
/// (`src/codegen/mod.rs:5885-5887`), while the type checker
...
/// `long long`.
#[test]
#[ignore = "XFAIL: an imported name that shadows a built-in is registered by the checkers but ignored by codegen — src/typeck/mod.rs:1593-1593 and the borrow checker insert the imported signature over the built-in, while src/codegen/mod.rs:5885-5887 tests is_builtin() first and unconditionally, so `pub fn print_int(s: String)` type-checks `print_int(\"hello\")` and then emits `__pd_print_int(\"hello\")`, which gcc rejects as \"incompatible pointer to integer conversion\" (owned by M4, cross-file module imports)"]
...
/// (`src/codegen/mod.rs:2146-2174`) and imported function bodies
/// (`src/codegen/mod.rs:2304-2316`) — from the prototype block
/// (`src/codegen/mod.rs:3083-3094`), which is a fourth site and emits
