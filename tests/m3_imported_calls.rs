...
// (`docs/contributing/MILESTONES.md:536-545`, "M4 — Modules", which claims the
...
/// re-derives visibility from `ast.items` instead: `src/typeck/mod.rs:558`,
/// `src/codegen/mod.rs:1218`, `src/codegen/mod.rs:1303`,
/// `src/codegen/mod.rs:1408`, `src/codegen/mod.rs:1899`, and the borrow checker's
...
/// (`src/codegen/mod.rs:1422-1431`) with no shadowing check at all. The front
...
/// A qualified call cannot be written. `src/parser/mod.rs:3039-3080` turns any
...
#[test]
#[ignore = "XFAIL: a qualified call `lib2::helper()` is unreachable — src/parser/mod.rs:3039-3080 turns every `a::b(...)` into Expr::EnumConstructor and src/typeck/mod.rs:2591-2596 then reports \"Undefined enum type: lib2\"; the same makes `import lib2 as m;` unusable, since `m::helper()` reports \"Undefined enum type: m\" (owned by M4, cross-file module imports)"]
...
/// (`src/codegen/mod.rs:2910-2913`), while the type checker
/// (`src/typeck/mod.rs:652`) and `register_imported_functions` both insert
...
/// `long long`.
#[test]
#[ignore = "XFAIL: an imported name that shadows a built-in is registered by the checkers but ignored by codegen — src/typeck/mod.rs:652 and the borrow checker insert the imported signature over the built-in, while src/codegen/mod.rs:2910-2913 tests is_builtin() first and unconditionally, so `pub fn print_int(s: String)` type-checks `print_int(\"hello\")` and then emits `__pd_print_int(\"hello\")`, which gcc rejects as \"incompatible pointer to integer conversion\" (owned by M4, cross-file module imports)"]
...
/// (`src/codegen/mod.rs:1303-1325`) and imported function bodies
/// (`src/codegen/mod.rs:1408-1420`) — from the prototype block
/// (`src/codegen/mod.rs:1901-1912`), which is a fourth site and emits
...
/// iterating `self.instantiations.keys()` (`src/typeck/mod.rs:3380`), which is a
/// `HashMap`, and `get_struct_instantiations` does the same
/// (`src/typeck/mod.rs:3442`). Codegen then emits in that Vec's order
/// (`src/codegen/mod.rs:1378`, `src/codegen/mod.rs:1916`).
///
...
/// (`src/typeck/mod.rs:3442`). Generic *structs* cannot be compiled at all right
