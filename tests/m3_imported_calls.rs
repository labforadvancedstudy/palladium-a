...
// (`src/codegen/mod.rs:1767-1767`) and so a skipped body "produces no C". That
...
// (`docs/contributing/MILESTONES.md:873-879`, "M4 — Modules", which claims the
...
// (`docs/contributing/MILESTONES.md:839`), which is not what these rows are
// about.
//
...
/// `src/codegen/mod.rs:1767-1767`, `src/codegen/mod.rs:1690-1690`,
/// `src/codegen/mod.rs:2000-2000`, `src/codegen/mod.rs:2833-2833`, and the borrow checker's
...
/// imported function (`src/codegen/mod.rs:2155-2165`) and emits the local one
/// unconditionally (`src/codegen/mod.rs:2172-2178`). This test is green.
///
...
/// `src/codegen/mod.rs:2161-2161`, and BOTH of the citations it leaned on had
...
/// A qualified call cannot be written. `src/parser/mod.rs:4108-4149` turns any
...
#[test]
#[ignore = "XFAIL: a qualified call `lib2::helper()` is unreachable — src/parser/mod.rs:4108-4149 turns every `a::b(...)` into Expr::EnumConstructor and src/typeck/mod.rs:4056-4060 then reports \"Undefined enum type: lib2\"; the same makes `import lib2 as m;` unusable, since `m::helper()` reports \"Undefined enum type: m\" (owned by M4, cross-file module imports)"]
...
/// (`src/codegen/mod.rs:5436-5438`), while the type checker
...
/// (`src/codegen/mod.rs:2003-2031`) and imported function bodies
/// (`src/codegen/mod.rs:2155-2167`) — from the prototype block
/// (`src/codegen/mod.rs:2830-2841`), which is a fourth site and emits
...
/// iterating `self.instantiations.keys()` (`src/typeck/mod.rs:5602-5602`), which is a
/// `HashMap`, and `get_struct_instantiations` does the same
/// (`src/typeck/mod.rs:5664-5664`). Codegen then emits in that Vec's order
/// (`src/codegen/mod.rs:1950-1950`, `src/codegen/mod.rs:2125-2125`).
///
...
/// (`src/typeck/mod.rs:5664-5664`). Generic *structs* cannot be compiled at all right
