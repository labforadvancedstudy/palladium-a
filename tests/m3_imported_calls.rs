...
// (`docs/contributing/MILESTONES.md:1085-1092`, "M4 — Modules", which claims the
...
// (`docs/contributing/MILESTONES.md:1025`), which is not what these rows are
// about.
//
...
/// A qualified call cannot be written. `src/parser/mod.rs:4427-4468` turns any
...
#[test]
#[ignore = "XFAIL: a qualified call `lib2::helper()` is unreachable — src/parser/mod.rs:4427-4468 turns every `a::b(...)` into Expr::EnumConstructor and src/typeck/mod.rs:4758-4762 then reports \"Undefined enum type: lib2\"; the same makes `import lib2 as m;` unusable, since `m::helper()` reports \"Undefined enum type: m\" (owned by M4, cross-file module imports)"]
...
/// iterating `self.instantiations.keys()` (`src/typeck/mod.rs:6482-6482`), which is a
/// `HashMap`, and `get_struct_instantiations` does the same
/// (`src/typeck/mod.rs:6544-6544`). Codegen then emits in that Vec's order
...
/// (`src/typeck/mod.rs:6544-6544`). Generic *structs* cannot be compiled at all right
