...
//! (`src/ownership/borrow_checker.rs:1292-1292`), and its first pass walks
...
//! `Expr::Ident` (`src/ownership/borrow_checker.rs:1292`), was looked up as a *value*,
...
// (`src/codegen/mod.rs:1218-1218`) and so a skipped body "produces no C". That
...
// (`docs/contributing/MILESTONES.md:498-498`, "M4 — Modules", which claims the
...
/// re-derives visibility from `ast.items` instead: `src/typeck/mod.rs:2591-2596`,
/// `src/codegen/mod.rs:1218`, `src/codegen/mod.rs:1218`,
/// `src/codegen/mod.rs:1218`, `src/codegen/mod.rs:1218`, and the borrow checker's
...
/// (`src/codegen/mod.rs:1218-1218`) and then every local one
/// (`src/codegen/mod.rs:1218-1218`) with no shadowing check at all. The front
...
/// A qualified call cannot be written. `src/parser/mod.rs:620-631` turns any
/// `a::b(...)` into `Expr::EnumConstructor`, and `src/typeck/mod.rs:2591-2596`
...
#[test]
#[ignore = "XFAIL: a qualified call `lib2::helper()` is unreachable — src/parser/mod.rs:620-631 turns every `a::b(...)` into Expr::EnumConstructor and src/typeck/mod.rs:2591-2596 then reports \"Undefined enum type: lib2\"; the same makes `import lib2 as m;` unusable, since `m::helper()` reports \"Undefined enum type: m\" (owned by M4, cross-file module imports)"]
...
/// resolver. `src/parser/mod.rs:620-631`: after `::`, if the token after the
...
/// (`src/codegen/mod.rs:1218-1218`), while the type checker
/// (`src/typeck/mod.rs:2591-2596`) and `register_imported_functions` both insert
...
/// `long long`.
#[test]
#[ignore = "XFAIL: an imported name that shadows a built-in is registered by the checkers but ignored by codegen — src/typeck/mod.rs:2591-2596 and the borrow checker insert the imported signature over the built-in, while src/codegen/mod.rs:1218-1218 tests is_builtin() first and unconditionally, so `pub fn print_int(s: String)` type-checks `print_int(\"hello\")` and then emits `__pd_print_int(\"hello\")`, which gcc rejects as \"incompatible pointer to integer conversion\" (owned by M4, cross-file module imports)"]
...
/// (`src/codegen/mod.rs:1218-1218`) and imported function bodies
/// (`src/codegen/mod.rs:1218-1218`) — from the prototype block
/// (`src/codegen/mod.rs:1218-1218`), which is a fourth site and emits
...
/// iterating `self.instantiations.keys()` (`src/typeck/mod.rs:2591-2596`), which is a
/// `HashMap`, and `get_struct_instantiations` does the same
/// (`src/typeck/mod.rs:2591-2596`). Codegen then emits in that Vec's order
/// (`src/codegen/mod.rs:1218`, `src/codegen/mod.rs:1218`).
///
...
/// (`src/typeck/mod.rs:2591-2596`). Generic *structs* cannot be compiled at all right
