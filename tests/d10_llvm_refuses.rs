...
/// codegen at `:184`). So under `--llvm` these two report the type checker's
...
// (`src/cli.rs:43-45`, `:61-63`) and the dispatch that turns the parsed flag
// into `driver.with_llvm()` (`src/main.rs:114-116`, `:149-151`). A test that
