...
#[test]
#[ignore = "XFAIL: `self` as an assignment target — docs/specification/grammar.ebnf:167 `place = identifier | place '[' … | place '.' … | '*' identifier` and `self` is a keyword (docs/specification/grammar.ebnf:52-56), not an identifier, so `self.data[0] = item` does not parse (owned by M2, surface syntax)"]
...
#[test]
#[ignore = "XFAIL: raw pointer types and `as` casts — grammar.ebnf:223 'no `as` casts'; there is no `*mut T` type either (owned by M4, 'A real reference type')"]
...
#[test]
#[ignore = "XFAIL: const generic parameters on an `impl` block — grammar.ebnf:119 admits `const N: T` and `fn`/`struct`/`enum` do parse it, but `parse_impl`'s parameter loop (src/parser/mod.rs:1538-1547) has no `const` arm and reports 'Expected type parameter name, but found const' (owned by M4, 'Generics that work')"]
...
#[test]
#[ignore = "XFAIL: inline `mod` blocks — grammar.ebnf:69-70 lists no module item, so `mod math { … }` is not an item (owned by M2, surface syntax)"]
