...
"#;
    ...
    let result = driver
        .compile_string(source, "missing_brace.pd")
        .map(|_| ());
    // In v0.1, we might get a generic "end of file" error instead of specific "brace" error
    assert!(
        result.is_err(),
        "Missing closing brace should cause parser error"
    );
    let error_msg = result.unwrap_err().to_string();
    assert!(
...
"#;
    ...
    let result = driver
        .compile_string(source, "missing_paren.pd")
        .map(|_| ());
    assert!(
        result.is_err(),
        "Missing opening parenthesis should cause parser error"
    );
}
...
"#;
    ...
    let result = driver
        .compile_string(source, "nested_errors.pd")
        .map(|_| ());
...
fn test_consecutive_errors() {
    ...
    assert!(
        result3.is_ok(),
        "Driver should recover from previous errors"
    );
}
...
fn test_meaningful_error_messages() {
    ...
    for (source, expected_words) in test_cases {
        let result = driver
            .compile_string(source, "error_msg_test.pd")
            .map(|_| ());
