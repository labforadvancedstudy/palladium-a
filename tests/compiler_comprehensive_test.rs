...
#[test]
#[ignore = "XFAIL: literal patterns in `match` — grammar.ebnf:263 'No literal patterns'; the parser reports \"Expected pattern, but found integer 1\" (owned by M2, item 4)"]
fn test_match_on_integer_literal() {
    compile_and_verify("fn main() { match 1 { 1 => {}, _ => {} } }", &["switch"]);
}

#[test]
#[ignore = "XFAIL: `trait` emits no C at all — grammar.ebnf:128 'Traits also emit no code'; there is no vtable mechanism anywhere in the compiler (owned by M4, 'Traits with real dispatch')"]
fn test_trait_declaration_emits_code() {
    compile_and_verify("trait Display { }\nfn main() { }", &["// Trait:"]);
}

#[test]
#[ignore = "XFAIL: top-level `const` items — grammar.ebnf:91-92 lists no const item, so the parser reports \"Expected function, struct, enum, trait, type, impl, or macro declaration\" (owned by M2, surface syntax)"]
...
#[test]
#[ignore = "XFAIL: top-level `static` items — grammar.ebnf:91-92 lists no static item, same parse error as `const` (owned by M2, surface syntax)"]
...
#[test]
#[ignore = "XFAIL: `loop` — grammar.ebnf:200 'and no `loop`'; the token does not exist, so the body's '{' is a parse error (owned by M2, item 3)"]
...
#[test]
#[ignore = "XFAIL: `as` casts — grammar.ebnf:245 'no `as` casts'; the parser stops at 'as' (owned by M2, surface syntax)"]
fn test_as_cast() {
...
#[test]
#[ignore = "XFAIL: bitwise operators — the expression grammar (grammar.ebnf:209-217) has no `&`/`|`/`^`/`<<`/`>>` level and the lexer has no '^' token at all (owned by M2, surface syntax)"]
...
#[test]
#[ignore = "XFAIL: `else if` — grammar.ebnf:192 'There is NO `else if`: after `else` the parser requires \\'{\\'' (owned by M2, item 2)"]
...
#[test]
#[ignore = "XFAIL: an `impl` block's `Self` is not a known type — this fixture is refused with 'Unknown struct type: Self' before it reaches the `&self` receiver and the method-call syntax (`x.f()`, grammar.ebnf:242, rejected as \"Indirect function calls not yet supported\") it was written for. All three are owed; this is the one that fires (owned by M4, 'Traits with real dispatch' / a real reference type)"]
